...
myApp.factory('postsFactory', ['$http', function($http) {
  ...
  function PostsFactory(){
    var _this = this;
    this.create = function(id, post, callback){
        console.log("INSIDE OF FACTORY")
        console.log(post)
        $http.post('/posts/' +id, post).then(function(returned_data){
    ...
    this.update = function(id ,newfriend, callback){ // what parameters do we need?
      $http.put('/posts/' +id, newfriend).then(function(returned_data){
            console.log(returned_data.data);
            if (typeof(callback) == 'function'){
                callback(returned_data.data);
            }
 ...
 //But only if you only want to run the callback from the controller.
    ...
    this.delete = function(id ,callback){// what parameters do we need?
        $http.delete('/posts/' +id).then(function(returned_data){
            console.log(returned_data.data);
            if (typeof(callback) == 'function'){
                callback(returned_data.data);
            }
